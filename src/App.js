...
class App extends React.Component {
  ...
  fetchTasks = () => {
    ...
    axios.get(url)
      ...
      .then(data => this.setState({
        taskList: data, loadingTasks: false
      })
      )
      .catch(error => {
        alert('Can\'t load tasks from the server');
      });

    console.log('Getting all tasks form the server... hello(TODO)');
    // TODO
  }
  ...
  createTask = () => {
    const { newTask } = this.state;
    const url = ' http://localhost:5000/tasks ';
    this.setState({ newTask: { ...newTask, _submitting: true } });
    axios.post(url, newTask)
      .then(res => res.data)
      .then(newTask => {
        this.setState({ taskList: [...this.state.taskList, { ...newTask }], newTask: { ...newTaskDefaultAttributes }, _submitting: false });
      });
  ...
  deleteTask = (id, name) => {
    ...
    axios.delete(url + id)
      ...
      .then(() => {
        ...
        this.setState({ alertOpen: true, alertMessage: `Task ${name} successfully deleted on the server !` });
      });
                ...
                : orderBy(taskList, ['createdAt'], ['desc']).map(t => {
                  ...
                  return (
                    <TableRow className={'task-row' + (processing ? ' processing' : '')} key={t.id}>
                      ...
                      <TableCell><p className='paragraph' title={hoverMessage}>{taskNameFieldValue}</p></TableCell>

                      <TableCell align='right'>
                        <DeleteForeverIcon className='task-action-btn' onClick={() => deleteTask(t.id, t.name)} />
